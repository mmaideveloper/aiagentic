                ...
                switch (ocrType)
                {
                    case string x when x.Equals("AzureAIDocIntel", StringComparison.OrdinalIgnoreCase):
                    {
                        AzureAIDocIntelConfig? cfg = appBuilder.Configuration
                            ...
                            .Get<AzureAIDocIntelConfig>() ?? throw new System.Configuration.ConfigurationException("Missing Azure AI Document Intelligence configuration");
...
#pragma warning restore KMEXP02 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
                    ...
                    case string x when x.Equals("Tesseract", StringComparison.OrdinalIgnoreCase):
                    {
                        TesseractConfig? cfg = appBuilder.Configuration
                            ...
                            .Get<TesseractConfig>() ?? throw new System.Configuration.ConfigurationException("Missing Tesseract configuration");
                        memoryBuilder.Services.AddSingleton(cfg);
